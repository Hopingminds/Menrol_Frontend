...
import { GoogleMap, LoadScript, InfoWindow, Autocomplete } from "@react-google-maps/api";
...
const Map: React.FC<MapProps> = () => {
  ...
  const userInfo: UserInfo | null = storedUserInfo ? JSON.parse(storedUserInfo) : null;
  ...
  const handleSaveAddress = async () => {
    if (selectedLocation && userInfo) {
      try {
        ...
        if (response.ok) {
            toast.success("Successfully Added Your Address!"); // Show success toast
            fetchSavedAddress(); // Fetch the latest saved address after saving a new one
          } else {
  ...
  const fetchSavedAddress = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const savedAddress = data.user.SavedAddresses[0]; // Access the first saved address from the array
  ...
  return (
    <LoadScript googleMapsApiKey="AIzaSyAmB63Ixx1tDyUyEvQ4KE1ymOM2YANXPn0" libraries={["places"]}>
      <div style={{ position: "relative", height: "100%" }}>
        <Autocomplete onLoad={(autocomplete) => setSearchBox(autocomplete)} onPlaceChanged={handlePlaceSelect}>
          <input
            ref={inputRef}
            type="text"
